...
| 合并转发 | 只有 `[合并转发]`；触发消息的引用甚至只渲染成英文段名 `[forward]` | `【未展开:合并转发7686537322889496857】`（**带 id**，可直接展开） |
...
5. 工具结果按条截断（`limit`），不要依赖外层 `TOOL_RESULT_MAX` 总截断。
6. **OneBot id 的类型**：`get_forward_msg(id)` 必须传**字符串**（forward id 是可能超出
   int32/JS 安全范围的长整型字符串，传 int 会被 NapCat 拒为「1200 消息已过期或者为内层消息」）；
   `get_msg(message_id)` 按契约传 int，失败时退回原始字符串重试。
